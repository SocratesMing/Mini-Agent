#!/usr/bin/env python3
"""
外汇和贵金属行情数据获取脚本 (从config.yaml读取配置版)
使用 akshare 库获取实时和历史外汇汇率和贵金属价格，并写入MySQL数据库

支持：
1. 实时数据获取
2. 近一个月历史数据获取
3. 从 config.yaml 读取数据库配置
"""
...
def get_db_config() -> dict:
    ...
    if db_config.type != "mysql":
        raise ValueError(f"数据库类型必须是 mysql，当前配置: {db_config.type}")
...
if __name__ == "__main__":
    print("外汇和贵金属行情数据获取脚本 (从config.yaml读取配置版)")
    print("="*60)
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--realtime":
            ...
            print("用法:")
            print("  python fetch_market_data_config.py --realtime   仅获取实时数据")
            print("  python fetch_market_data_config.py --history    仅获取历史数据")
            print("  python fetch_market_data_config.py --show       显示所有数据")
    else:
        fetch_realtime()
        show_realtime_data()
